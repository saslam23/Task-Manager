...
mongoose.connect("mongodb://localhost:27017/taskmanagerDB", { useNewUrlParser: true, useUnifiedTopology: true, useCreateIndex: true })
...
app.get('/lists', (req, res) => {
    List.find()
        ...
        .catch(err => {
            res.status(200).json({ message: 'successfully retrieved Lists' })
            console.log(err)
        })
})
...
app.delete('/lists/:listId/tasks/:taskId', (req, res) => {
    Task.findByIdAndDelete(req.params.taskId)
        .then(res.json({ message: 'successfully deleted Task' }))
        .catch(res.status(500).json({ error: 'something went wrong' }))
})
